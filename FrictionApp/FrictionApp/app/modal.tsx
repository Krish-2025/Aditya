import { addTask, generateTodayInstances, getInstanceById, getLocalDateStr, initDatabase, isTaskNameTaken, updateInstance } from '@/services/database';
...
export default function CreateTaskModal() {
  ...
  // Validation States
  const [taskNameTaken, setTaskNameTaken] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);

  useEffect(() => { initDatabase().catch(() => { }); }, []);
  ...
  // Check unique task name
  useEffect(() => {
    if (taskName.length > 0) {
      isTaskNameTaken(taskName).then(taken => setTaskNameTaken(taken));
    } else {
      setTaskNameTaken(false);
    }
  }, [taskName]);
  ...
  const getValidationState = (name: string, value: string) => {
    ...
    if (name === 'name') return taskNameTaken ? 'error' : 'success';
    if (name === 'hours') {
  ...
  const nameVal = getValidationState('name', taskName);
  ...
  const handleSave = async () => {
    if (!taskName || !hours || !minutes) {
      Alert.alert('Missing Fields', 'Please fill out Task Name and Scheduled Time.');
      return;
    }
    if (nameVal === 'error') {
      Alert.alert('Duplicate Name', `A task named "${taskName}" already exists.`);
      return;
    }
    ...
    try {
      ...
      const instanceId = `${taskName}::${todayStr}`;
      ...
      setTimeout(() => router.back(), 150); // Small delay for effect
    } catch (e) {
  ...
  const renderValidationIcon = (name: string, valState: 'none' | 'success' | 'error') => {
    if (focusedInput === name) return null;
    if (valState === 'error') return <Ionicons name="alert-circle" size={18} color="#EF4444" style={styles.valIcon} />;
    if (valState === 'success' && name === 'name') return <Ionicons name="checkmark-circle" size={18} color="#22C55E" style={styles.valIcon} />;
    return null;
  };
        ...
        <BlurView intensity={80} tint="dark" style={styles.glassCard}>
          ...
          <ScrollView style={styles.scrollContainer} contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.inputWrapper}>
              <TextInput
                style={getInputStyle('name', nameVal)}
              ...
              />
              {renderValidationIcon('name', nameVal)}
            </View>
            {nameVal === 'error' && <Text style={styles.errorText}>This task name already exists.</Text>}
            ...
            <View style={styles.bottomActions}>
              ...
              <TouchableOpacity style={styles.submitBtnWrapper} onPress={handleSave} disabled={isSubmitting}>
                ...
                >
                  <Text style={styles.submitText}>{isSubmitting ? 'Saving...' : 'Create Task'}</Text>
...
const styles = StyleSheet.create({
  ...
  glassCard: {
    ...
    overflow: 'hidden',
    backgroundColor: 'rgba(15, 18, 24, 0.65)', // deeply tinted black base
  },
  header: {
    flexDirection: 'row',
